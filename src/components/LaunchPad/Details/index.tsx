...
const LaunchpadDetails = () => {
  ...
  const {
    ...
    error,
    isValidating,
  } = useSWR(`/api/v1/presales/${id}`, fetcher);
  ...
  console.log("myContribution", data?.myContribution);
  const details = useMemo<IInfoList[]>(() => {
    ...
    let arr: IInfoList[] = [
      ...
      },
      {
        title: "Total Supply",
        value: `${
          data.token?.totalSupply && Math.round(data.token?.totalSupply)
        } ${data.token?.symbol}`,
      },
      {
        title: "Presale Rate",
        value: `1 ${data.payment_currency?.symbol} ~ ${formatPrice(
      ...
      {
        title: "Listing Type",
        value: data.listingType === "0" ? "Auto" : "Manual",
      },
      {
        title: "Refund Type",
      ...
      },
      {
        ...
        value: `${(data.actualRaised && data.actualRaised / 10 ** 18) ?? 0}`,
      },
      {
