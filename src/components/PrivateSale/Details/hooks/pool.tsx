...
export const usePool = (update: { address: string }): IPoolDetails => {
  ...
  useEffect(() => {
    if (newData) {
      ...
      const poolContributor = poolContributors.find(
        (a: { id: string | undefined }) => a.id === account?.toLowerCase()
      );
      const poolDetailsSplit = pool?.poolDetails?.toString().includes("$#$")
        ? pool.poolDetails.toString().split("$#$")
        : false;

      const userTier = poolContributor?.tier;

      setStats({
        tier1:
          pool.tier1EndTime && pool.tier1EndTime > 0
            ? {
            ...
            : undefined,
        tier2:
          pool.tier2EndTime && pool.tier2EndTime > 0
            ? {
                start_time: pool.tier1EndTime,
                end_time: pool.tier2EndTime,
              }
            : undefined,
        userWhitelisted: !!poolContributor?.whitelisted,
        ...
        //   100,
        tokenSupply:
          pool.token?.totalSupply / Math.pow(10, pool.token?.decimals),
        ...
        currencySymbol: supportNetwork[chainId || "default"]
          ? supportNetwork[chainId || "default"].symbol
          : supportNetwork["default"].symbol,
        whitelists: poolContributors
          .filter((con: { whitelisted: any }) => con.whitelisted)
          .map((con: { id: any }) => ({
            address: con.id,
          })),
      });
